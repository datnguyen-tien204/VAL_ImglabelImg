...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        filterate = action(get_str('filterate'), self.filterate, 'F8', 'edit')
...
######################################################################################
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding, self.menus.recentFiles,
                     save, save_format, save_as, close, reset_all, delete_image, quit))
    ...
    def show_shortcuts_dialog(self):
        self.show_tutorial_dialog(browser='default', link='https://github.com/T-SW/AutoLabelImg#hotkeys')
    ...
    def load_file(self, file_path=None):
        ...
        with open(self.cfg_path, 'r', encoding='utf8') as f:
            ...
            data['Last loaded image name'] = file_path.split('\\')[-1]
    ...
    def change_save_dir_dialog(self, _value=False):
        ...
        if dir_path:
            with open(self.cfg_path, 'r', encoding='utf8') as f:
                data = yaml.load(f, Loader=yaml.FullLoader)
                data['Last loaded label folder'] = dir_path
            with open(self.cfg_path, 'w') as f:
                yaml.dump(data, f)
    ...
    def open_dir_dialog(self, _value=False, dir_path=None, silent=False):
        ...
        if target_dir_path:
            with open(self.cfg_path, 'r', encoding='utf8') as f:
                data = yaml.load(f, Loader=yaml.FullLoader)
                data['Last loaded image folder'] = target_dir_path
            with open(self.cfg_path, 'w') as f:
                yaml.dump(data, f)

################################################################################
    def backhaul_conf(self, value):
        if self.conf != value:
            self.conf = value
            self.download_updates()

    def backhaul_iou(self, value):
        if self.iou != value:
            self.iou = value
            self.download_updates()

    def backhaul_classes(self, value):
        if self.classes != value:
            self.classes = value
            self.download_updates()

    def parameter_settings(self):
        self.Sub_panel_p = Sub_panel_parameter(self.conf, self.iou, self.classes)  # 自定义窗口
        self.Sub_panel_p.signal_conf.connect(self.backhaul_conf)
        self.Sub_panel_p.signal_iou.connect(self.backhaul_iou)
        self.Sub_panel_p.signal_classes.connect(self.backhaul_classes)
        self.Sub_panel_p.show()

    def model_selection(self):
        weight_path = QFileDialog.getOpenFileName(self,
                                                  "getOpenFileName", "./models",
                                                  "Weight Files (*.pt)")
        if len(weight_path[0]) == 0:
            return
        else:
            self.model = weight_path[0]
            self.download_updates()

    def load_data(self):
        if not os.path.exists(self.cfg_path):
            QMessageBox.information(self, u'Wrong!', u'have no cfg file(default.yaml), please check again.')
            return
        with open(self.cfg_path, 'r', encoding='utf8') as f:
            data = yaml.load(f, Loader=yaml.FullLoader)
        image_folder_path, label_folder_path, image_name = data['Last loaded image folder'], data['Last loaded label folder'], data['Last loaded image name']
        if image_folder_path and os.path.isdir(image_folder_path):
            self.import_dir_images(image_folder_path)
            print("Load Image Folder!")
        else:
            print("No image folder path or image folder does not exist!")
        if label_folder_path and os.path.isdir(label_folder_path):
            self.default_save_dir = label_folder_path
            print("Load Save Folder!")
        else:
            print("No save folder path or save folder does not exist!")
        if image_folder_path and label_folder_path and image_name:
            image_path = os.path.join(image_folder_path, image_name)
            if os.path.exists(image_path):
                self.cur_img_idx = self.m_img_list.index(image_path.replace('/', '\\'))
    ...
    def filterate(self):
        with open(self.cfg_path, 'r', encoding='utf8') as f:
            data = yaml.load(f, Loader=yaml.FullLoader)
        self.filter_conf, self.check_ls = data['filter_conf'], data['check_ls']
        self.repeat_labels = filter_main(self.last_open_dir, self.default_save_dir, self.filter_conf, self.check_ls)
        QMessageBox.information(self, 'Prompt', 'Filter completed!')

    def load_imagels_prev(self):    # repeat_labels  repeat_labels_sign
        if self.repeat_labels_sign > 0:
            self.repeat_labels_sign -= 1
        self.load_file(self.repeat_labels[self.repeat_labels_sign])

    def load_imagels_next(self):
        if self.repeat_labels_sign < len(self.repeat_labels)-1:
            self.repeat_labels_sign += 1
        self.load_file(self.repeat_labels[self.repeat_labels_sign])
    ...
    def auto_label(self):
        ...
        fname = file.split('.')[0]
        lab_path = f"{self.default_save_dir}/{fname}.txt"
        ...
        try:
            ...
            QMessageBox.information(self, 'FileNotFoundError!', f'please check file:{fname}, its shape is {img_shape}.')
    ...
    def auto_label_all(self):
        if not self.filepath:
            QMessageBox.information(self, 'Wrong!', 'have no loaded folder yet, please check again.')
            return
        option = ["Current image and after", "All images", "Index range of images"]
        value, ok = QInputDialog.getItem(self, "Auto Label All", "Select Mode:", option, 2, False)
        if ok and value == "Current image and after":
            detect_images = self.m_img_list[self.m_img_list.index(self.filepath.replace('/', '\\')):]
        elif ok and value == "All images":
            detect_images = self.m_img_list
        elif ok and value == "Index range of images":
            interval, ok = QInputDialog.getText(self, "Index range of images", "input:num(start)-num(end)", QLineEdit.Normal)
            if ok and len(interval) > 0:
                try:
                    start, end = interval.split('-')
                    detect_images = self.m_img_list[int(start)-1:int(end)]
        ...
        del_labels_list = [os.path.join(self.default_save_dir, path.split('\\')[-1].replace('.jpg', '.txt').replace('.png', '.txt')) for path in detect_images]
        ...
        progress.setLabelText("Auto Labeling...")
        progress.setMinimumDuration(5)
        progress.setWindowModality(Qt.WindowModal)
        length = len(detect_images)
        ...
        for i in range(length):
            ...
            try:
                ...
                # model.predict(detect_images[i], save_txt=True, save=True, device=self.device)
                labpath = os.path.join(self.default_save_dir, detect_images[i].split('\\')[-1].replace('.jpg', '.txt'))
                rework_classes(labpath)
            except FileNotFoundError:
                from PIL import Image
                import numpy as np

                file = detect_images[i].split('\\')[-1]
                img = Image.open(detect_images[i])
                img_shape = np.asarray(img).shape
                error += f"Please check file:{file}, its shape is {img_shape}.\n"
                error_ls.append(detect_images[i])
            if progress.wasCanceled():
                QMessageBox.warning(self, "prompt", "Auto Labeling interruption!")
                break
        else:
        ...
        if error_ls:
            ...
            ok = QMessageBox.question(self, "Solution", "Do you want to delete the files?", QMessageBox.Yes | QMessageBox.No)
            if ok:
                for path in error_ls:
                    os.remove(path)
    ...
    def train_test_split(self):
        ...
        # OPEN DIALOG TO SELECT LABEL FOLDER
        ...
        #  LABEL TO SHOW TOTAL RATIO for train set
        ...
        #  LABEL TO SHOW TOTAL RATIO for val set
        ...
        # LABEL TO SHOW TOTAL RATIO for test set
    ...
    def split_data(self, image_folder, label_folder, train_ratio, val_ratio, test_ratio, selected_format):
        ...
        labels = glob.glob(os.path.join(label_folder, "*.txt" if "VOC" in selected_format else "*.xml"))

        # SPILT DATA with ratio
    ...
    def convert_and_move_files(self, image_paths, label_paths, split_type, selected_format, output_folder,status_button):
        ...
        if(status_button==True):
            ...
            ## Add zip_folder button
            zip_button = QPushButton("Zip Files", msg_box)
            msg_box.addButton(zip_button, QMessageBox.ActionRole)
            zip_button.clicked.connect(lambda: (self.zip_files(output_folder), msg_box.accept()))
    ...
    ## ZIP FILES FUNCTION
    def zip_files(self, folder_path):
        zip_save_path, _ = QFileDialog.getSaveFileName(self, "Save Zip File", "", "Zip Files (*.zip)")
        if zip_save_path:
            ...
            zipf.close()
            QMessageBox.information(self, 'Successfully', 'Zip file created successfully.')
    ...
    def voc_to_yolo(self, xml_file, output_folder):
        ...
        # List all classes in the XML file
        class_list = []

        for obj in root.iter('object'):
            name = obj.find('name').text
            if name not in class_list:
                class_list.append(name)  # Add class if not already in list
        ...
        yolo_label = ""
        output_file_name = os.path.basename(xml_file)[:-4]  # Get the XML file name without extension
        ...
        for obj in root.iter('object'):
            ...
            if name in class_list:
                ...
                # Convert bounding box from VOC to YOLO format
                box_x = (x_min + x_max) / 2.0 / w
                box_y = (y_min + y_max) / 2.0 / h
                box_w = (x_max - x_min) / w
                box_h = (y_max - y_min) / h

                cls_id = class_list.index(name)

                # Add bounding box info to yolo_label
                yolo_label += f'{cls_id} {box_x:.6f} {box_y:.6f} {box_w:.6f} {box_h:.6f}\n'

        # WRITE LABEL TO FILE
        with open(f'{output_folder}/classes.txt', 'w') as class_file:
    ...
    def yolo_to_voc(self, txt_file, image_file, temp_folder):
        # IF EXIST CLASSES.TXT CHOOSE IT, ELSE OPEN FILE DIALOG
    ...
    def create_default(self):
        config = {
            ...
            'Last loaded label folder': None,
            'Last loaded image name': None,
            'conf': 0.5,
            'iou': 0.85,
            'classes': [2, 5, 7],
    ...
    def read_updates(self):
        ...
        with open(self.cfg_path, 'r', encoding='utf8') as f:
            data = yaml.load(f, Loader=yaml.FullLoader)
        self.conf, self.iou, self.classes, self.model = data['conf'], data['iou'], data['classes'], data['model']
        self.filter_conf, self.check_ls = data['filter_conf'], data['check_ls']

    def download_updates(self):
        config = {
            'Last loaded image folder': self.last_open_dir,
            'Last loaded label folder': self.default_save_dir,
            'Last loaded image name': self.filepath,
            'conf': self.conf,
            'iou': self.iou,
            'classes': self.classes,
            'model': self.model,
            'filter_conf': self.filter_conf,
            'check_ls': self.check_ls
        }
...
class Sub_panel_parameter(QWidget):

    signal_conf, signal_iou = pyqtSignal(float), pyqtSignal(float)
    signal_classes = pyqtSignal(list)

    def __init__(self, conf, iou, classes):
        super().__init__()
        ...
        input_box_qhbox_layout_conf = QHBoxLayout()  # 输入框
        input_box_qhbox_layout_iou = QHBoxLayout()
        input_box_qhbox_layout_classes = QHBoxLayout()
        # message_window_qhbox_layout = QHBoxLayout()  # 消息窗口

        self.conf, self.iou, self.classes = conf, iou, classes
        qhbox_layout_conf = QPushButton('conf')
        qhbox_layout_iou = QPushButton('iou')
        qhbox_layout_classes = QPushButton('classes')

        # for btn in (qhbox_layout_conf, qhbox_layout_iou, qhbox_layout_classes):
        #     input_box_qhbox_layout.addWidget(btn)
        input_box_qhbox_layout_conf.addWidget(qhbox_layout_conf)
        input_box_qhbox_layout_iou.addWidget(qhbox_layout_iou)
        input_box_qhbox_layout_classes.addWidget(qhbox_layout_classes)

        qhbox_layout_conf.clicked.connect(self.change_conf)
        qhbox_layout_iou.clicked.connect(self.change_iou)
        qhbox_layout_classes.clicked.connect(self.change_classes)

        list_layout.addLayout(input_box_qhbox_layout_conf)
        list_layout.addLayout(input_box_qhbox_layout_iou)
        list_layout.addLayout(input_box_qhbox_layout_classes)

    def change_conf(self):
        new_conf, ok = QInputDialog.getDouble(self, 'change conf', f"conf({self.conf})", float(self.conf), 0.01, 0.99, 2)
        self.judge_parameter(new_conf, ok, 'conf')

    def change_iou(self):
        new_iou, ok = QInputDialog.getDouble(self, 'change iou', f"conf({self.iou})", float(self.iou), 0.01, 0.99, 2)
        self.judge_parameter(new_iou, ok, 'iou')

    def change_classes(self):
        new_classes, ok = QInputDialog.getText(self, "change classes", f"classes({str(self.classes)[1:-1]})", QLineEdit.Normal)
        self.judge_parameter(new_classes, ok, 'classes')

    def judge_parameter(self, parameter, ok, parameter_name):
        type_float_parameter_name = ['conf', 'iou']
        if parameter_name in type_float_parameter_name:
            if ok and parameter and parameter_name == 'conf':
                self.conf = float(parameter)
                self.signal_conf.emit(self.conf)
            elif ok and parameter and parameter_name == 'iou':
                self.iou = float(parameter)
                self.signal_iou.emit(self.iou)
            else:
                return
        elif parameter_name == 'classes':
            if ok and len(parameter) > 0:
                self.classes = [int(i) for i in parameter if i.isdigit()]
                self.signal_classes.emit(self.classes)
            else:
                return
